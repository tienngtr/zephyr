...
int fido2_crypto_export_keypair(uint32_t key_id, uint8_t *private_key, size_t private_key_size,
				size_t *private_key_len)
{
	psa_status_t status;

	status = psa_export_key(key_id, private_key, private_key_size, private_key_len);
	if (status != PSA_SUCCESS) {
		LOG_ERR("Private key export failed key_id=0x%08x status=%d", key_id, status);
	}

	return status;
}

int fido2_crypto_import_keypair(const uint8_t *private_key, size_t private_key_len,
				uint32_t *key_id)
{
	psa_key_attributes_t attr = PSA_KEY_ATTRIBUTES_INIT;
	psa_key_id_t id;
	...
	status = psa_import_key(&attr, private_key, private_key_len, &id);
	psa_reset_key_attributes(&attr);
	if (status != PSA_SUCCESS) {
		LOG_ERR("Private key import failed status=%d", status);
		return status;
	}
