...
/* Ressembly state. We build these up from RX init / cont*/
...
static int usb_hid_submit_packet(const uint8_t *pkt)
{
	...
	deadline = k_uptime_get() + CONFIG_FIDO2_CTAPHID_TIMEOUT_MS;
	while (atomic_test_and_set_bit(hid_ep_in_busy, HID_EP_BUSY_FLAG)) {
	...
	ret = hid_int_ep_write(hid_dev, pkt, CTAPHID_PACKET_SIZE, &wrote);
	if (ret || wrote != CTAPHID_PACKET_SIZE) {
			      ...
			      const size_t len)
{
	...
	put_cid(pkt, cid);
	pkt[4] = cmd | BIT(7); /* MSB bit high = init, low = continue */
...
static void fido2_hid_int_in_ready(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static void fido2_hid_int_out_ready(const struct device *dev)
{
	uint8_t pkt[CTAPHID_PACKET_SIZE];
	uint32_t read;
	int ret;

	ret = hid_int_ep_read(dev, pkt, sizeof(pkt), &read);
	if (ret) {
		LOG_WRN("HID OUT read failed: %d", ret);
		return;
	}

	fido2_hid_output_report(dev, read, pkt);
}

static int fido2_hid_get_report(const struct device *dev, struct usb_setup_packet *setup,
...
static int fido2_hid_set_report(const struct device *dev, struct usb_setup_packet *setup,
				int32_t *len, uint8_t **data)
{
	uint8_t report_type = (setup->wValue >> 8) & 0xff;

	if (report_type == HID_REPORT_TYPE_OUTPUT) {
		fido2_hid_output_report(dev, *len, *data);
	}

	return 0;
}

static const struct hid_ops fido2_hid_ops = {
	.get_report = fido2_hid_get_report,
	.set_report = fido2_hid_set_report,
	.int_in_ready = fido2_hid_int_in_ready,
	.int_out_ready = fido2_hid_int_out_ready,
};

/* Public API */
static int usb_hid_transport_init(fido2_transport_recv_cb_t cb, void *user_data)
{
	int ret;
	...
	if (usb_hid_set_proto_code(hid_dev, HID_BOOT_IFACE_CODE_NONE)) {
		LOG_WRN("Failed to set HID protocol code");
	}
...
static const struct fido2_transport_api usb_hid_api = {.init = usb_hid_transport_init,
						       .send = usb_hid_send,
