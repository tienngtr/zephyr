...
#define GET_ASSERTION_RESP_NUM_CREDS  0x05
			       ...
			       size_t cbor_out_cap, size_t *cbor_out_len)
{
	 ...
	 * absent = unsupprted. So we check with num_pin_uv_auth_protocols
	 */
	if (info->num_pin_uv_auth_protocols > 0) {
...
static int decode_tstr_field(zcbor_state_t *zs, char *out, size_t out_cap)
{
	...
	size_t copy_len = MIN(str.len, out_cap - 1); /* -1 to hold NUL */

	memcpy(out, str.value, copy_len);
	out[copy_len] = '\0';
...
static int mc_decode_exclude_list(zcbor_state_t *zs, struct fido2_make_credential_params *params)
{
	...
	while (!zcbor_array_at_end(zs)) {
		...
		if (valid_type && params->num_exclude < FIDO2_MAX_ALLOW_LIST) {
...
static int ga_decode_allow_list(zcbor_state_t *zs, struct fido2_get_assertion_params *params)
{
	...
	while (!zcbor_array_at_end(zs)) {
		...
		if (valid_type && params->num_allow < FIDO2_MAX_ALLOW_LIST) {
			if (cred_id.len > FIDO2_CREDENTIAL_ID_MAX_SIZE) {
				return -EBADMSG;
			}
					 ...
					 size_t *cbor_out_len)
{
	...
	if (!zcbor_map_start_encode(zs, map_count)) {
		return -ENOMEM;
	}
	...
	if (!zcbor_tstr_put_lit(zs, "id") ||
	    !zcbor_bstr_encode_ptr(zs, cred->id, cred->id_len)) {
		return -ENOMEM;
	}
	...
	if (include_user) {
		uint8_t user_map_count = 1;

		if (cred->user_name[0] != '\0') {
			user_map_count++;
		}
		if (cred->user_display_name[0] != '\0') {
			user_map_count++;
		}

		if (!zcbor_uint32_put(zs, GET_ASSERTION_RESP_USER) ||
		    !zcbor_map_start_encode(zs, user_map_count)) {
			return -ENOMEM;
		}
		if (!zcbor_tstr_put_lit(zs, "id") ||
		    !zcbor_bstr_encode_ptr(zs, cred->user_id, cred->user_id_len)) {
			return -ENOMEM;
		}
		if (cred->user_name[0] != '\0' &&
		    (!zcbor_tstr_put_lit(zs, "name") ||
		     !zcbor_tstr_put_term(zs, cred->user_name, sizeof(cred->user_name)))) {
			return -ENOMEM;
		}
		if (cred->user_display_name[0] != '\0' &&
		    (!zcbor_tstr_put_lit(zs, "displayName") ||
		     !zcbor_tstr_put_term(zs, cred->user_display_name,
					  sizeof(cred->user_display_name)))) {
			return -ENOMEM;
		}
		if (!zcbor_map_end_encode(zs, user_map_count)) {
			return -ENOMEM;
		}
	}

	if (num_credentials > 1 &&
	    (!zcbor_uint32_put(zs, GET_ASSERTION_RESP_NUM_CREDS) ||
	     !zcbor_uint32_put(zs, num_credentials))) {
		return -ENOMEM;
	}

	if (!zcbor_map_end_encode(zs, map_count)) {
		return -ENOMEM;
	}
