...
static int mc_populate_credential(void)
{
	...
	sys_rand_get(mc_credential.id, mc_credential.id_len);
		       ...
		       size_t *cbor_out_len, const struct fido2_transport *transport, uint32_t cid)
{
	...
	ret = fido2_crypto_export_keypair(mc_credential.key_id, mc_credential.private_key,
					  sizeof(mc_credential.private_key),
					  &private_key_len);
	if (ret) {
		fido2_crypto_destroy_key(mc_credential.key_id);
		return FIDO2_ERR_OTHER;
	}
	mc_credential.private_key_len = private_key_len;
