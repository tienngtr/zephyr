...
struct settings_load_one_ctx {
	void *data;
	size_t len;
	ssize_t ret;
};

static int fido2_settings_load_one_cb(const char *key, size_t len, settings_read_cb read_cb,
				      void *cb_arg, void *param)
{
	struct settings_load_one_ctx *ctx = param;
	...
	ctx->ret = read_cb(cb_arg, ctx->data, ctx->len);

	return 0;
}

static ssize_t fido2_settings_load_one(const char *key, void *data, size_t len)
{
	struct settings_load_one_ctx ctx = {
		.data = data,
		.len = len,
		.ret = -ENOENT,
	};
	...
	ret = settings_load_subtree_direct(key, fido2_settings_load_one_cb, &ctx);
	if (ret) {
		return ret;
	}
...
static int cred_slot_get(const uint8_t *cred_id, size_t cred_id_len, struct fido2_credential *out)
{
	...
	for (int i = 0; i < CONFIG_FIDO2_MAX_CREDENTIALS; ++i) {
		...
		if (fido2_settings_load_one(key, &cred, sizeof(cred)) != sizeof(cred)) {
			continue;
		}
...
static int restore_credential_key(struct fido2_credential *cred)
{
	if (cred->private_key_len != FIDO2_ES256_PRIVATE_KEY_SIZE) {
		return -EINVAL;
	}

	return fido2_crypto_import_keypair(cred->private_key, cred->private_key_len,
					   &cred->key_id);
}
...
static int settings_backend_store(const struct fido2_credential *cred)
{
	...
	for (int i = 0; i < CONFIG_FIDO2_MAX_CREDENTIALS; ++i) {
		...
		build_key(key, sizeof(key), i);
		if (fido2_settings_load_one(key, &temp, sizeof(temp)) == sizeof(temp)) {
				 ...
				 struct fido2_credential *cred)
{
	...
	return restore_credential_key(cred);
}
				   ...
				   struct fido2_credential *cred)
{
	...
	if (cred != NULL) {
		ret = restore_credential_key(cred);
		if (ret) {
			return ret;
		}
	}
	...
	*count = 0;
	...
	for (int i = 0; i < CONFIG_FIDO2_MAX_CREDENTIALS; ++i) {
		...
		for (int j = 0; j < i; ++j) {
			...
			if (fido2_settings_load_one(prev_key, &prev, sizeof(prev)) != sizeof(prev)) {
				continue;
			}
...
static int settings_backend_pin_get(uint8_t pin_hash[FIDO2_PIN_HASH_SIZE])
{
	...
	ret = fido2_settings_load_one(FIDO2_SETTINGS_PIN_HASH, pin_hash, FIDO2_PIN_HASH_SIZE);
...
static int settings_backend_pin_retries_get(uint8_t *retries)
{
	...
	ret = fido2_settings_load_one(FIDO2_SETTINGS_PIN_RETRIES, retries, sizeof(*retries));
