...
/** @brief Credential extension credBlob */
...
enum fido2_status {
	...
	FIDO2_ERR_FP_DATABASE_FULL = 0x17,        /**< Fingerprint database full */
	FIDO2_ERR_LARGE_BLOB_STORAGE_FULL = 0x18, /**< Large blob storage full */
	...
	FIDO2_ERR_RESERVED = 0x38,                /**< PIN/UV auth token expired */
...
#define FIDO2_ES256_PRIVATE_KEY_SIZE 32
...
struct fido2_credential {
	...
	uint32_t key_id;
	/** Exported ES256 private key for storage backends without persistent PSA keys */
	uint8_t private_key[FIDO2_ES256_PRIVATE_KEY_SIZE];
	/** Exported private key length */
...
struct fido2_device_info {
	...
	struct {
		...
		bool cred_mgmt;            /**< Credential management support */
		bool pin_uv_auth_token;    /**< pinUvAuthToken support */
		bool make_cred_uv_not_rqd; /**< makeCredUvNotRqd support */
		bool always_uv;            /**< Authenticator always requires UV */
		bool no_mc_ga_permissions_with_client_pin; /**< Only if clientPin present */
	} options;
	/** Firmware version */
