...
#define ZEPHYR_INCLUDE_AUTHENTICATION_FIDO2_FIDO2_STORAGE_H_
...
struct fido2_storage_api {
	...
	int (*update_user_info)(const uint8_t *cred_id, size_t cred_id_len, const char *user_name,
...
/**
 * @brief  Increment and return the signature counter.
 * @param cred_id     Credential ID bytes
 * @param cred_id_len Length of credential ID
...
/**
 * @brief Update user information for an existing credential.
 * @param cred_id Credential ID bytes
 * @param cred_id_len Length of credential ID
 * @param user_name New user name (NULL to leave unchanged)
 * @param user_display_name New user display name (NULL to leave unchanged)
 * @retval 0 On success
 * @retval -ENOENT If credential not found
 */
...
/**
 * @brief Get the total number of stored discoverable credentials.
...
#endif /* ZEPHYR_INCLUDE_AUTHENTICATION_FIDO2_FIDO2_STORAGE_H_ */
